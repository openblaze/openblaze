module.exports = async (tx) => {
    ...
    if (state.senateCandidates[tx.input.senatorPubkey].votes.length >= threshold) {
        state.senators[tx.input.senatorPubkey] = { ...state.senateCandidates[tx.input.senatorPubkey], lastRewardsClaim: Math.round(Object.values(state.clock).reduce((pv, cv) => pv + cv, 0) / state.clock) }
    }
}